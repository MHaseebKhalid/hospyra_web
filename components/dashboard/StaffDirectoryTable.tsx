...
import type { StaffDirectoryTableItem } from '@/components/dashboard/TablesStaticData';

const StaffDirectoryTable = ({ rows }: { rows: StaffDirectoryTableItem[] }) => {
        ...
        <table className="w-full border-collapse">
          <thead className="bg-[#F3F4F6] border-b border-t border-[#E5E7EB]">
            <tr className="text-left text-sm text-[#52525B] font-effra">
              ...
              <th className="px-5 py-4 font-medium">Hourly Wage</th>
              <th className="px-5 py-4 font-medium">Weekly Hours</th>
          ...
          <tbody>
            {pageRows.map((row) => (
              <tr key={row.id} className="border-b border-[#E5E7EB] text-sm text-[#27272A] font-effra">
                ...
                <td className="px-5 py-4">{row.hourlyWage}</td>
                <td className="px-5 py-4">{row.weeklyHours}</td>
                <td className="px-5 py-4">
                  <span className="inline-flex rounded-md px-3 py-1 text-xs font-semibold bg-black text-white">
                    {row.status}
                  </span>
                </td>
                <td className="px-5 py-4">
